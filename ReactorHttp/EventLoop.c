...
int readLocalMessage(void *arg)
{
    ...
    char buf[256];
    read(evLoop->socketPair[1], buf, sizeof(buf)); // 触发了读事件后 检测的poll epoll select就解除阻塞了
    return 0;
}
...
struct EventLoop *eventLoopInitEx(const char *threadName)
{
    ...
    evLoop->dispatcher = &SelectDispatcher; // 这里使用的是select
...
int eventLoopRun(struct EventLoop *evLoop)
{
    ...
    while (!evLoop->isQuit)
    {
        dispatcher->dispatch(evLoop, 2); // 超时时长 2s //dispatch()内会调用回调函数
...
int eventLoopAddTask(struct EventLoop *evLoop, struct Channel *channel, int type)
{
    ...
    if (evLoop->threadID == pthread_self())
    {
        // 当前子线程(基于子线程的角度分析)调用添加任务函数 --> 去处理任务
        eventLoopProcessTask(evLoop);
    }
    else
    {
        // 主线程调用添加任务函数 --> 告诉子线程处理任务队列中的任务
        // 1. 子线程在工作 2. 子线程被阻塞了:select, poll, epoll
        taskWakeup(evLoop); // 往socketPair[0]里写数据 socketPair[1]就被激活了 其channel对应的读回调readLocalMessage就会被调用
    }
    return 0;
}
...
int eventLoopProcessTask(struct EventLoop *evLoop)
{
    ...
    while (head != NULL)
    {
        ...
        if (head->type == ADD)
        {
            // 添加     //文件描述符在channel中 需要处理的数据在evLoop中
