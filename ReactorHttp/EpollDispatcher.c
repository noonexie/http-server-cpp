...
{
    int epfd;                   // epoll树的根节点
    struct epoll_event *events; // epoll wait 传出参数：检测到的有效文件描述符放到这个数组
};
...
static void *epollInit()
{
    ...
    data->events = (struct epoll_event *)calloc(Max, sizeof(struct epoll_event)); // Max树上的节点数	// calloc分配内存后自动初始化为0
...
static int epollDispatch(struct EventLoop *evLoop, int timeout)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (events & EPOLLERR || events & EPOLLHUP) // 异常事件
        {
            // 对方断开了连接, 删除 fd
