...
void tcpServerRun(struct TcpServer *server)
{
    ...
    struct Channel *channel = channelInit(server->listener->lfd,
                                          ReadEvent, acceptConnection, NULL, NULL, server); // 第一个参数=文件描述符 第二个参数=监听文件描述符的什么事件 第三个参数=读事件触发调用的函数
    eventLoopAddTask(server->mainLoop, channel, ADD);                                       // eg:这个函数最终将主线程文件描述的监听事件添加到epoll树上进行监听
    // 启动主线程反应堆模型
